...
async function apiGet(params){
  const url=new URL(API_BASE_URL); Object.entries(params||{}).forEach(([k,v])=>url.searchParams.set(k,v));
  const resp=await fetch(url.toString(),{method:"GET"}); const text=await resp.text();
...
async function apiText(params){
  const url=new URL(API_BASE_URL); Object.entries(params||{}).forEach(([k,v])=>url.searchParams.set(k,v));
  const resp=await fetch(url.toString(),{method:"GET"}); const text=await resp.text();
  if(!resp.ok) throw new Error(HTTP ${resp.status}: ${text.slice(0,200)}); return text;
}
function setDateDefault(input, deltaDays=0){ const d=new Date(); d.setDate(d.getDate()+deltaDays); input.value=d.toISOString().slice(0,10); }
...
async function loadReferentials(){
  ...
  // Remplir sélecteurs d'équipe
  [document.getElementById("b_equipe"), document.getElementById("c_equipe")].forEach(sel=>{
    if(!sel) return; sel.innerHTML="";
    REF_EQ.forEach(e=>{ const o=document.createElement("option"); o.value=e; o.textContent=e; sel.appendChild(o); });
  });
}
...
function b_addRow(matDefault="", cibleDefault=""){
  ...
  const sel=document.createElement("select"); sel.className="b_mat";
  sel.innerHTML=<option value="">— choisir —</option> + REF_MAT.map(m=><option value="${escapeHtml(m)}">${escapeHtml(m)}</option>).join("");
  if(matDefault) sel.value=matDefault; tdMat.appendChild(sel);
  // Cible
  const tdCible=document.createElement("td");
  const inp=document.createElement("input"); inp.type="number"; inp.min="0"; inp.step="1"; inp.value=cibleDefault||""; inp.className="b_cible";
  tdCible.appendChild(inp);
  // Commentaire
  const tdCom=document.createElement("td");
  const txt=document.createElement("input"); txt.type="text"; txt.placeholder="commentaire (facultatif)"; txt.className="b_comment";
  tdCom.appendChild(txt);
  // Del
  const tdDel=document.createElement("td");
  const btn=document.createElement("button"); btn.textContent="✕"; btn.className="secondary"; btn.addEventListener("click",()=>tr.remove());
  tdDel.appendChild(btn);

  tr.append(tdMat,tdCible,tdCom,tdDel); tbody.appendChild(tr);
}
...
async function loadPlan(dateJ1){
  ...
  document.getElementById("r_details").innerHTML=toTable(["Équipe","Zone","Matériel","Reste Veille","Cible","Besoin"], det);
  const agg=(r.agregat||[]).map(x=>[x[1],x[2]]);
  document.getElementById("r_agregat").innerHTML=toTable(["Matériel","Quantité"], agg);
}
...
function parseTextLinesToRows(txt){
  const out=[]; (txt||"").split(/\r?\n/).forEach(line=>{
    const m=line.split(","); if(m.length>=1){
      const mat=(m[0]||"").trim(); const q=m.length>=2?parseInt((m[1]||"").trim(),10)||0:0;
...
async function ensureXLSXLoaded(){
  ...
  await new Promise((res,rej)=>{ const s=document.createElement("script");
    ...
    s.onload=res; s.onerror=()=>rej(new Error("CDN XLSX introuvable")); document.head.appendChild(s);
  });
}
...
async function apiGetDashboard(J,J1,F,T){
  try{ return await apiGet({ dashboard:"1", date:J, jplus1:J1, from:F, to:T }); }
  catch(e){ const msg=String(e&&e.message||e);
    if(msg.includes("Paramètre")) return await apiGet({ dashboard:"summary", j:J, j1:J1, from:F, to:T });
    throw e;
  }
}
function kpiTotalFromBlock(b){ if(!b) return 0; if(typeof b.total==="number") return b.total;
  return (b.rows||[]).reduce((s,r)=>s+(parseInt(r?.[1],10)||0),0);
}
async function loadDashboard(){
  ...
  try{
    ...
    msg.textContent=J=${r.dates?.J} • J+1=${r.dates?.J1} • Usage: ${r.dates?.from}→${r.dates?.to}; msg.className="ok";
    window._DASH_LAST_=r;
  }catch(e){ msg.textContent="Erreur: "+e.message; msg.className="error"; }
}
async function exportDashboardXLSX(){
  const r=window._DASH_LAST_; if(!r){ alert("Charge d’abord le Dashboard."); return; }
  await ensureXLSXLoaded(); const wb=XLSX.utils.book_new();
  const add=(name,blk)=>{ if(!blk) return; const data=[(blk.headers||[]),...(blk.rows||[])]; const ws=XLSX.utils.aoa_to_sheet(data);
    XLSX.utils.book_append_sheet(wb,ws,name.slice(0,31)); };
  add("Stock_VC",r.stockVC); add("EntreesBib_J",r.entreesBiblioJour); add("EntreesBib_J+1_plan",r.entreesBiblioJplus1_plan);
  add("EntreesBib_J+1_reel",r.entreesBiblioJplus1_reelles); add("Repartition_J",r.repartJourEquipes); add("Besoins_J+1",r.besoinsJplus1Equipes); add("Usage",r.usagePivot);
  const J=document.getElementById("dashJ").value||"J"; XLSX.writeFile(wb,dashboard_${J}.xlsx);
}
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  document.getElementById("c_equipe").addEventListener("change",()=>{
    ...
    const z=document.getElementById("c_zone"); if(!z.value) z.value=v;
  });
